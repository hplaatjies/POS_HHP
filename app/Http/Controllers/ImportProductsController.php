...
class ImportProductsController extends Controller
{
        	...
        	ini_set('max_execution_time', 0);
        		...
        		foreach ($imported_data as $key => $value) {
					...
					if( $product_array['type'] == 'single' ){
						...
						if(!empty($selling_price_string)){
							$selling_price = array_map([$this->productUtil, 'num_uf'], 
						...
						if(!empty($profit_margin_string)){
							$profit_margin = array_map([$this->productUtil, 'num_uf'], 
											array_map('trim', explode('|', 
						...
						$array_lengths_count = array(count($variation_values), count($dpp_inc_tax), count($dpp_exc_tax), count($selling_price), count($profit_margin));
						...
						foreach ($variation_values as $k => $v) {
							...
							//Assign Values
							$product_array['variation']['variations'][] = array(
								'value' => $v,
						...
						if(!empty($value[20]) && $enable_stock == 1){
							...
							foreach ($variation_os as $key => $value) {
								$product_array['variation']['variations'][$key]['opening_stock'] = $value;
								$product_array['variation']['variations'][$key]['opening_stock_expiry_date'] = null;
								
								if(!empty($value[22])){
									$product_array['variation']['variations'][$key]['opening_stock_exp_date'] = \Carbon::createFromFormat('m-d-Y', trim($value[22]))->format('Y-m-d');
								} else {
									$product_array['variation']['variations'][$key]['opening_stock_exp_date'] = null;
								}
							}
        		...
        		if(!empty($formated_data)){
        			foreach ($formated_data as $index => $product_data) {
        				...
        				//Rack, Row & Position.
        				$this->rackDetails($imported_data[$index][25], 
        					$imported_data[$index][26], $imported_data[$index][27], $business_id, $product->id, $index+1);
    ...
    private function calculateVariationPrices( $dpp_exc_tax, $dpp_inc_tax, $selling_price, $tax_amount, $tax_type, $margin ){
    	...
    	if($dpp_inc_tax == 0 ){
			$dpp_inc_tax = $this->productUtil->calc_percentage($dpp_exc_tax, $tax_amount, 
				$dpp_exc_tax);
		}
		...
		if( $selling_price != 0 ){
			if( $tax_type == 'inclusive' ){
				...
				$dsp_exc_tax = $this->productUtil->calc_percentage_base($dsp_inc_tax, 
								$tax_amount);

			} elseif ($tax_type == 'exclusive') {
				$dsp_exc_tax = $selling_price;
				$dsp_inc_tax = $this->productUtil->calc_percentage($selling_price, $tax_amount, 
								$selling_price);
			}
		} else {
			$dsp_exc_tax = $this->productUtil->calc_percentage($dpp_exc_tax, 
				$margin, $dpp_exc_tax);
			$dsp_inc_tax = $this->productUtil->calc_percentage($dsp_exc_tax, $tax_amount, 
							$dsp_exc_tax);
		}
    ...
    private function addOpeningStockForVariable($variations, $product, $business_id){
        ...
        if(!empty($variations['variations'][0]['opening_stock'])){
        	//Add opening stock transaction
    		$transaction = Transaction::create(
                            array(
                                'type' => 'opening_stock',
