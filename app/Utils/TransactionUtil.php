...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false)
    {
        ...
        foreach ($products as $product) {
            ...
            if(!empty($product['transaction_sell_lines_id'])){
                ...
                $lines_formatted[] = new TransactionSellLine([
        ...
        foreach ($payments as $payment) {
            ...
            if(!empty($payment['payment_id'])){
                ...
                if($this->num_uf($payment['amount']) != 0){
                    $payments_formatted[] = new TransactionPayment([
     ...
     * @param array $receipt_printer_type
     *
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        foreach($transaction->sell_lines as $line)
        {
            ...
            if(!empty($tax_details)){
                if($tax_details->is_tax_group){
                    ...
                    if(!isset($output['taxes'][$tax_name])){
                        $output['taxes'][$tax_name] = 0;
                    }
                    $output['taxes'][$tax_name] += ($line->quantity * $line->item_tax);
                }
            }
            ...
            $line_array = [
                ...
                'name' => $product->name,
                'variation' => ($variation->name == 'DUMMY') ? '' : $variation->name,
                //Field for 2nd column
                'quantity' => $line->quantity,
                'units' => $unit->short_name,
            ...
            if($il->show_sku == 1){
                $line_array['sub_sku'] = $variation->sub_sku;
            }
        ...
        //Shipping charges
        $output['shipping'] = ($transaction->shipping_charges != 0) ? $this->num_f($transaction->shipping_charges, $show_currency) : 0;
        ...
        if(!empty($il->module_info['Restaurant'])){
            if($this->isModuleEnabled('Restaurant')){

                //Table label & info
                $output['table_label'] = null;
                $output['table'] = null;
                if(isset($il->module_info['Restaurant']['show_table'])){
                    $output['table_label'] = !empty($il->module_info['Restaurant']['table_label']) ? $il->module_info['Restaurant']['table_label'] : '';
                    if(!empty($transaction->res_table_id)){
                        $table = \Modules\Restaurant\Entities\ResTable::find($transaction->res_table_id);   
                    }
                ...
                //Waiter label & info
                $output['waiter_label'] = null;
                $output['waiter'] = null;
                if(isset($il->module_info['Restaurant']['show_waiter'])){
                    $output['waiter_label'] = !empty($il->module_info['Restaurant']['waiter_label']) ? $il->module_info['Restaurant']['waiter_label'] : '';
                    if(!empty($transaction->res_waiter_id)){
                        $waiter = \App\User::find($transaction->res_waiter_id);   
                    }
                    
                    //res_table_id
                    $output['waiter'] = !empty($waiter->id) ? implode(' ', [$waiter->first_name, $waiter->last_name]) : '';
                }
            }
        }
    ...
    {
        $query = Transaction::leftJoin('transaction_payments as tp', 'transactions.id', '=', 
                                    'tp.transaction_id')
                        ->where('business_id', $business_id )
                        ->where('type', 'purchase')
                        ->select('final_total', 
     ...
     * @return Obj
     */
    public function getExpenseReport($business_id, $filters = array(), 
        $type = 'by_category')
    {
     ...
     * @return object
     */
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $rows = $query->select('PL.id as purchase_lines_id', 
            ...
            if(! ($qty_selling == 0 || is_null($qty_selling))){
                ...
                if($mapping_type == 'purchase'){
                    $mismatch_error = trans("messages.purchase_sell_mismatch_exception", 
                    ...
                    $mismatch_error = trans("messages.purchase_stock_adjustment_mismatch_exception", 
                    ['product' => $mismatch_name]);
                }
     ...
     * @return void
     */
    public function adjustMappingPurchaseSell($status_before, $transaction, 
        $business, $deleted_line_ids = []){
    ...
    public function adjustMappingPurchaseSellAfterEditingPurchase($before_status, $transaction, $delete_purchase_lines){
        ...
        if($before_status == 'received' && $transaction->status == 'received'){
            ...
            $purchase_lines = Transaction::join('purchase_lines AS PL', 
                        'transactions.id', '=', 'PL.transaction_id')
                    ->join('transaction_sell_lines_purchase_lines AS TSPL', 
                        'PL.id', '=', 'TSPL.purchase_line_id')
        ...
        foreach ($purchase_lines as $purchase_line) {
            ...
            $tspl = TransactionSellLinesPurchaseLines::where('purchase_line_id', $purchase_line['id'])
                        ->leftjoin('transaction_sell_lines AS SL', 
                        'transaction_sell_lines_purchase_lines.sell_line_id', '=', 'SL.id')
                        ->leftjoin('stock_adjustment_lines AS SAL', 
                        'transaction_sell_lines_purchase_lines.stock_adjustment_line_id', '=', 'SAL.id')
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false){
        ...
        $query = PurchaseLine::join('transactions as purchase',
                    'purchase_lines.transaction_id', '=', 'purchase.id')
    ...
    public function getTotalStockAdjustment($business_id, $start_date = null, $end_date = null, $location_id = null){
        ...
        $q = Transaction::where('business_id', $business_id)
                        ...
                        ->select(DB::raw("SUM(final_total) as total_adjustment"),
