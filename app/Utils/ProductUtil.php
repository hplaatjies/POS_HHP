...
class ProductUtil extends Util
{
    ...
    {
        $types = array( 'C128' => 'Code 128 (C128)', 'C39' => 'Code 39 (C39)', 'EAN13' => 'EAN-13', 'EAN8' => 'EAN-8', 'UPCA' => 'UPC-A', 'UPCE' => 'UPC-E');
	        ...
	        if(!empty($value['variations'])){
	            ...
	            foreach($value['variations'] as $k => $v){
	                $variation_data[] = array(
                      'name' => $v['value'],
                      'product_id' => $product->id,
                      'sub_sku' => $this->generateSubSku($product->sku, $c, $product->barcode_type),
    ...
    public function updateVariableProductVariations($product_id, $input_variations_edit)
    {
        ...
        foreach( $input_variations_edit as $key => $value){
            ...
            if(!empty($value['variations'])){
                ...
                $variation_data = array();
                $c = Variation::withTrashed()->count() + 1;
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null )
    {
        ...
        $products = $query->select(DB::raw("IF(pv.is_dummy = 0, CONCAT(p.name, 
     ...
     * @return Obj
     */
    public function getTrendingProducts($business_id, $filters = array())
    {
        $query = Transaction::join( 'transaction_sell_lines as tsl', 
        ...
        $products = $query->select( DB::raw("SUM(tsl.quantity) as total_unit_sold"),
                                    'p.name as product',
    ...
    public function getDetailsFromProduct($business_id, $product_id, $variation_id = null)
    {
        ...
        $products = $product->select('products.id as product_id', 
                        'products.name as product_name', 'v.id as variation_id', 'v.name as variation_name')
                    ->get();
